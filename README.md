...
* **Layered Caching:** Implemented a **dual-layer L1 (Local) and L2 (Redis) caching strategy** to serve frequent requests. This reduces primary database load and speeds up lookups for frequently accessed links.

* **Pluggable Persistence:** Persistent URL storage is abstracted behind a common storage interface, allowing the service to use either **AWS DynamoDB or PostgreSQL** without changing the application or API layer.
...
3. **Pluggable Persistent Storage:** Introduced a generic `Store` interface that abstracts persistent URL operations from the underlying database implementation. The service can use either DynamoDB or PostgreSQL as its persistent storage backend, selected through configuration.
...
3. **Persistence Layer:** **AWS DynamoDB** was used as the persistent storage backend during these benchmarks.

> **Note:** The benchmarks below specifically measure the application using DynamoDB as its persistence backend. PostgreSQL support was added later and is not represented in these measurements.
...
* **Persistence Backend:** AWS DynamoDB
